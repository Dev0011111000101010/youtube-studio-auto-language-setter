...
### Docstring и коммиты
- Docstring после `def` — на **русском**, кратко (одна строка)
- Первая строка файла — относительный путь от корня проекта (если формат поддерживает комментарий)
- Коммиты — на **русском**

### Обратная связь с браузером
Любая функция, взаимодействующая с браузером, обязана:

1. **Читать DOM после действия** — не просто выполнить goto/click, а прочитать реальный элемент со страницы как подтверждение
2. **Ждать элемент явно** — `wait_for_selector(selector, timeout=5000)`, не `wait_for_load_state`
3. **Логировать прочитанное** — `log(f"[DOM] {что прочитали}: '{значение}' (URL: {page.url})")`
4. **Падать с ошибкой** если элемент не появился — не молчать, писать в лог и возвращать `False`
...
try:
    ...
    log(f"[DOM] {описание}: '{value}' (URL: {page.url})")
except PlaywrightTimeoutError:
    log(f"ОШИБКА: {selector} не появился за 5 сек (URL: {page.url})")
    return False
```
